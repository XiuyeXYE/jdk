 ...
 * <code>com.sun.jdi.EventRequestManager.createAccessWatchpointRequest()</code> <BR>
 ...
 *    - The watchpoint watches accesses to the specified field. <BR>
 *    - Throws: UnsupportedOperationException -                 <BR>
 *              if the target virtual machine does not support  <BR>
 *              this operation.                                 <BR>
 * <BR>
...
public class craccwtchpreq003 extends JDIBase {
    ...
    public static int run (String argv[], PrintStream out) {
        ...
        int exitCode = new craccwtchpreq003().runThis(argv, out);
    ...
    */
    ...
    //------------------------------------------------------  testing section
        ...
        AccessWatchpointRequest awRequest1 = null;
        AccessWatchpointRequest awRequest2 = null;
        ...
        for (int i = 0; ; i++) {
            ...
            if ( vm.canWatchFieldAccess() ) {
                log2("......vm.canWatchFieldAccess == true");
                log2("      creating two AccessWatchpointRequests for 'field': awRequest1 & awRequest2");

                awRequest1 = eventRManager.createAccessWatchpointRequest(field);
                awRequest2 = eventRManager.createAccessWatchpointRequest(field);

                log2("      checking up on: requests are disabled");
                if (awRequest1.isEnabled()) {
                    testExitCode = FAILED;
                    log3("ERROR: awRequest1 is enabled");
                }
                if (awRequest2.isEnabled()) {
                    testExitCode = FAILED;
                    log3("ERROR: awRequest2 is enabled");
                }
                log2("      compareing field equality");
                if ( !awRequest1.field().equals(field) ) {
                    testExitCode = FAILED;
                    log3("ERROR: !awRequest1.field().equals(field)");
                }
                if ( !awRequest2.field().equals(field) ) {
                    testExitCode = FAILED;
                    log3("ERROR: !awRequest2.field().equals(field)");
                }

            } else {
                log2("......vm.canWatchFieldAccess == false");
