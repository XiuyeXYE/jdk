 ...
 * <code>com.sun.jdi.EventRequestManager.createBreakpointRequest()</code> <BR>
 * complies with its spec.                                      <BR>
 * <BR>
 * The test checks up on the following assertion:                       <BR>
 *    - Multiple breakpoints at the same location are permitted.        <BR>
 *    - Creates a new disabled BreakpointRequest.                       <BR>
 *    - The BreakpointRequest is at the specified location.             <BR>
 * <BR>
...
public class crbreakpreq003 extends JDIBase {
    ...
    public static int run (String argv[], PrintStream out) {
        ...
        int exitCode = new crbreakpreq003().runThis(argv, out);
    ...
    //------------------------------------------------------  testing section
        ...
        for (int i = 0; ; i++) {
            ...
            //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ variable part
                ...
                log2("      creating two BreakpointRequests for 'breakpLocation': bpRequest1 & bpRequest2");

                bpRequest1 = eventRManager.createBreakpointRequest(breakpLocation);
                bpRequest2 = eventRManager.createBreakpointRequest(breakpLocation);

                log2("      checking up on: requests are disabled");
                if (bpRequest1.isEnabled()) {
                    testExitCode = FAILED;
                    log3("ERROR: bpRequest1 is enabled");
                }
                if (bpRequest2.isEnabled()) {
                    testExitCode = FAILED;
                    log3("ERROR: bpRequest2 is enabled");
                }
                log2("      compareing locations equality");
                if ( !bpRequest1.location().equals(breakpLocation) ) {
                    testExitCode = FAILED;
                    log3("ERROR: !bpRequest1.location().equals(breakpLocation)");
                }
                if ( !bpRequest2.location().equals(breakpLocation) ) {
                    testExitCode = FAILED;
                    log3("ERROR: !bpRequest2.location().equals(breakpLocation)");
                }
