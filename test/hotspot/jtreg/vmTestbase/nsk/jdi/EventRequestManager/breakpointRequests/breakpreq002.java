 ...
 * <code>com.sun.jdi.EventRequestManager.breakpointRequests()</code> <BR>
 * complies with its spec.                                      <BR>
 * <BR>
 * The test checks up on the following assertion:                       <BR>
 * - The list is unmodifiable.                                          <BR>
 * - The list of the enabled and disabled breakpoint requests.          <BR>
...
public class breakpreq002 extends JDIBase {
    ...
    public static int run (String argv[], PrintStream out) {
        ...
        int exitCode = new breakpreq002().runThis(argv, out);
    ...
    //------------------------------------------------------  testing section
        ...
        for (int i = 0; ; i++) {
            ...
            for (int i1 = 0; i1 < bpRequests.length; i1++) {
                ...
                log2("......checking up on returned List after creating new BreakpointRequest");
                ...
                while (li.hasNext()) {
                    request = (BreakpointRequest) li.next();
                ...
                log2("      enabling created BreakpointRequest");
                bpRequests[i1].enable();
                requests = eventRManager.breakpointRequests();
                listSize = requests.size();
                if ( listSize != (i1 + 2) ) {
                    testExitCode = FAILED;
            ...
            log2("......deleting BreakpointRequests");
            for (int i2 = bpRequests.length -1; i2 >= 0; i2--) {
                eventRManager.deleteEventRequest(bpRequests[i2]);
                requests = eventRManager.breakpointRequests();
                listSize = requests.size();
                if ( listSize != (i2 + 1) ) {
                    testExitCode = FAILED;
