...
package nsk.jdi.EventRequestManager.vmDeathRequests;
 ...
 * <code>com.sun.jdi.EventRequestManager.vmDeathRequests()</code> <BR>
 * complies with its spec.                                      <BR>
 * <BR>
 * The test checks up on the following assertion:                       <BR>
 * - The list is unmodifiable.                                          <BR>
 * - The list of the enabled and disabled VM death requests.            <BR>
...
public class vmdeathreq001 extends JDIBase {
    ...
    public static int run (String argv[], PrintStream out) {
        ...
        int exitCode = new vmdeathreq001().runThis(argv, out);
    ...
    */
    ...
    //------------------------------------------------------  testing section
        ...
        VMDeathRequest request = null;
        VMDeathRequest vmRequests[] = { null, null, null, null, null,
        ...
        for (int i = 0; ; i++) {
            ...
            log2("......creating VMDeathRequests");
            for (int i1 = 0; i1 < vmRequests.length; i1++) {
                vmRequests[i1] = eventRManager.createVMDeathRequest();
                vmRequests[i1].putProperty("number", String.valueOf(i1));

                log2("......checking up on returned List after creating new VMDeathRequest");
                requests = eventRManager.vmDeathRequests();
                ...
                while (li.hasNext()) {
                    request = (VMDeathRequest) li.next();
                ...
                log2("      enabling created VMDeathRequest");
                vmRequests[i1].enable();
                requests = eventRManager.vmDeathRequests();
                listSize = requests.size();
                if ( listSize != (i1 + 1) ) {
                    testExitCode = FAILED;
            ...
            log2("......deleting VMDeathRequests");
            for (int i2 = vmRequests.length -1; i2 >= 0; i2--) {
                eventRManager.deleteEventRequest(vmRequests[i2]);
                requests = eventRManager.vmDeathRequests();
                listSize = requests.size();
                if ( listSize != i2 ) {
                    testExitCode = FAILED;
