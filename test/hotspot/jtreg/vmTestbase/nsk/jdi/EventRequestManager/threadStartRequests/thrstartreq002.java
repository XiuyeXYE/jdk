 ...
 * <code>com.sun.jdi.EventRequestManager.threadStartRequests()</code> <BR>
 * complies with its spec.                                      <BR>
 * <BR>
 * The test checks up on the following assertion:                       <BR>
 * - The list is unmodifiable.                                          <BR>
 * - The list of the enabled and disabled thread start requests.        <BR>
...
public class thrstartreq002 extends JDIBase {
    ...
    public static int run (String argv[], PrintStream out) {
        ...
        int exitCode = new thrstartreq002().runThis(argv, out);
    ...
    */
    ...
    //------------------------------------------------------  testing section
        ...
        ThreadStartRequest request = null;
        ThreadStartRequest tsRequests[] = { null, null, null, null, null,
        ...
        for (int i = 0; ; i++) {
            ...
            log2("......creating ThreadStartRequests");
            for (int i1 = 0; i1 < tsRequests.length; i1++) {
                tsRequests[i1] = eventRManager.createThreadStartRequest();
                tsRequests[i1].putProperty("number", String.valueOf(i1));

                log2("......checking up on returned List after creating new ThreadStartRequest");
                requests = eventRManager.threadStartRequests();
                ...
                while (li.hasNext()) {
                    request = (ThreadStartRequest) li.next();
                ...
                log2("      enabling created ThreadStartRequest");
                tsRequests[i1].enable();
                requests = eventRManager.threadStartRequests();
                listSize = requests.size();
                if ( listSize != (i1 + 1) ) {
                    testExitCode = FAILED;
            ...
            log2("......deleting ThreadStartRequests");
            for (int i2 = tsRequests.length -1; i2 >= 0; i2--) {
                eventRManager.deleteEventRequest(tsRequests[i2]);
                requests = eventRManager.threadStartRequests();
                listSize = requests.size();
                if ( listSize != i2 ) {
                    testExitCode = FAILED;
