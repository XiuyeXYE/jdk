 ...
 *  - If neither caught exception nor uncaught exception is selected,   <BR>
 *  that is, both notifyCaught and notifyUncaught arguments are false,  <BR>
 *  returned value is null.                                             <BR>
 * Note. In condition of no Exception for such testcase is defined in   <BR>
 * the spec,   null is the only reasonable returned value.              <BR>
 * <BR>
 ...
 * - The debuggee prepares a testcase and invokes                       <BR>
...
public class crexreq010 extends JDIBase {
    ...
    public static int run (String argv[], PrintStream out) {
        ...
        int exitCode = new crexreq010().runThis(argv, out);
    ...
    //------------------------------------------------------  testing section
        ...
        for (int i = 0; ; i++) {
            ...
            try {
                ...
                exRequest = eventRManager.createExceptionRequest(refType, false, false);
                log2("      check on if returned value is null");
                if (exRequest == null) {
                    log3("ERROR: returned value is null!");
                    testExitCode = FAILED;
                }
            } catch (Exception e) {
                log3("ERROR: exception caught: " + e);
                testExitCode = FAILED;
            }
