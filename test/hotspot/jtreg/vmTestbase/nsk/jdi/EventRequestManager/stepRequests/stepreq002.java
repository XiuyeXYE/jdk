 ...
 * <code>com.sun.jdi.EventRequestManager.stepRequests()</code> <BR>
 * complies with its spec.                                      <BR>
 * <BR>
 * The test checks up on the following assertion:                       <BR>
 * - The list is unmodifiable.                                          <BR>
 * - The list of the enabled and disabled step requests.                <BR>
...
public class stepreq002 extends JDIBase {
    ...
    */
    ...
    //------------------------------------------------------  testing section
        ...
        StepRequest stRequests[] = { null, null, null, null, null,
        ...
        for (int i = 0; ; i++) {
            ...
            for (int i1 = 0; i1 < stRequests.length; i1++) {
                stRequests[i1] = eventRManager.createStepRequest( (ThreadReference) threads.getValue(i1),
                                                                 StepRequest.STEP_MIN, StepRequest.STEP_OUT);
                stRequests[i1].putProperty("number", String.valueOf(i1));

                log2("......checking up on returned List after creating new StepRequest");
                requests = eventRManager.stepRequests();
                ...
                log2("      enabling created StepRequest");
                stRequests[i1].enable();
                requests = eventRManager.stepRequests();
                listSize = requests.size();
                if ( listSize != (i1 + 1) ) {
                    testExitCode = FAILED;
            ...
            log2("......deleting StepRequests");
            for (int i2 = stRequests.length -1; i2 >= 0; i2--) {
                eventRManager.deleteEventRequest(stRequests[i2]);
                requests = eventRManager.stepRequests();
                listSize = requests.size();
                if ( listSize != i2 ) {
                    testExitCode = FAILED;
