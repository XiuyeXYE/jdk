 ...
 * <code>com.sun.jdi.ThreadReference.frames_ii()</code>         <BR>
 ...
 * - checks up that invocation ThreadReference.frames(,)                <BR>
 *     doesn't throw 'IncompatibleThreadStateException' when            <BR>
 *     the thread2 is suspended, and does throw otherwize;              <BR>
 * - sets up breakpoints within thread2's 'run', 'runt1', and           <BR>
 *   'runt2' methods providing the thread2 will be suspended within     <BR>
 *   'runt1' two times, before to call 'runt2' and after return from it;<BR>
 * - forces main thread to leave the synchronized block in order to     <BR>
 *   unlock the thread2 which will be suspended at the breakpoints.     <BR>
 *                                                                      <BR>
 * At the breakpoints the debugger performs a number of checks as follows.<BR>
 * (1) The breakpoint within 'run':                                     <BR>
 *     - frameList = thread2.frames(0, 1); doesn't throw an exception   <BR>
 *       and      frameList.size() == 1                                 <BR>
 *     - the sequence of method calls 'frame>location>method'           <BR>
 *       doesn't throw exception                                        <BR>
 * (2) First breakpoint within 'runt1':                                 <BR>
 *     - no exceptions thrown for: frameList = thread2.frames(0, 2);    <BR>
 *                                 frameList = thread2.frames(1, 1);    <BR>
 *     - additionally, two Method objects, mirroring gebuggee's methods <BR>
 *       'run' and 'runt1' are created for further check                <BR>
 * (3) The breakpoint within 'runt2':                                   <BR>
 *     - no exceptions thrown for several correct pairs of              <BR>
 *       the arguments index and lenght                                 <BR>
 * (4) Second breakpoint within 'runt1':                                <BR>
 *     - another pair of Method objects, mirroring gebuggee's methods   <BR>
 *       'run' and 'runt1', is created , and                            <BR>
 *       first mirror objects are compared to second mirror objects.    <BR>
 * <BR>
 */

public class frames_ii001 {
    ...
    public static int run (String argv[], PrintStream out) {
        return new frames_ii001().runThis(argv, out);
    }
    ...
    private String testedClassName =
        "nsk.jdi.ThreadReference.frames_ii.Threadframes_ii001a";
    ...
    //------------------------------------------------------  testing section
        ...
        for (int i = 0; ; i++) {
            ...
            label1: {
                ...
                Method runt1Method1 = null;
                Method runt1Method2 = null;
                ...
                for (int i3 = 0; i3 < 4; i3++) {
                    ...
                    switch (i3) {
                      ...
                      case 0:
                              log2("......a breakpoint within 'run'");
                              log2("         case for check: thread2.frames(0, 1);");
                              frameList = null;
                              try {
                                  frameList = thread2.frames(0, 1);
                                  ...
                                  log2("           checking up on: method calls: frame>location>method");
                                  try {
                                      frame      = (StackFrame) frameList.get(0);
                                      location   = frame.location();
                                      runMethod0 = location.method();
                                  } catch ( Exception e ) {
                              ...
                              log2("         case for check: frameList = thread2.frames(0, -1);");
                              try {
                                  frameList = thread2.frames(0, -1);
                      ...
                      case 1:
                              ...
                              try {
                                  ...
                                  log2("           checking up on: frameList.size() == 2");
                                  if (frameList.size() != 2) {
                                      log3("ERROR: frameList.size() != 2 : " + frameList.size());
                                      expresult = returnCode1;
                                  }
                                  ...
                                  try {
                                      frame        = (StackFrame) frameList.get(0);
                                      location     = frame.location();
                                      runt1Method1 = location.method();

                                      frame        = (StackFrame) frameList.get(1);
                                      location     = frame.location();
                                      runMethod1   = location.method();
                                  } catch ( Exception e ) {
                                      log3("ERROR: exception : " + e);
                                      expresult = returnCode1;
                                  }
                              ...
                              log2("         case for check: frameList = thread2.frames(1, 1);");
                              frameList = null;
                              try {
                      ...
                      case 2:
                              ...
                              log2("         case for check: thread2.frames(0, 3);");
                              frameList = null;
                              try {
                                  frameList = thread2.frames(0, 3);

                                  log2("           checking up on: frameList.size() == 3");
                                  if (frameList.size() != 3) {
                                      log3("ERROR: frameList.size() != 3 : " + frameList.size());
                                      expresult = returnCode1;
                                  }
                              } catch ( Exception e ) {
                                  log3("ERROR: Exception : " + e);
                              ...
                              log2("         case for check: frameList = thread2.frames(1, 2);");
                              frameList = null;
                              try {
                                  frameList = thread2.frames(1, 2);
                              ...
                              log2("         case for check: frameList = thread2.frames(2, 1);");
                              frameList = null;
                              try {
                                  frameList = thread2.frames(2, 1);
                      ...
                      case 3:
                              ...
                              try {
                                  ...
                                  try {
                                      frame = (StackFrame) frameList.get(0);
                                      location = frame.location();
                                      runt1Method2 = location.method();

                                      frame = (StackFrame) frameList.get(1);
                                      location = frame.location();
                                      runMethod2 = location.method();
                                  } catch ( Exception e ) {
                                      log3("ERROR: Exception : " + e);
                                      expresult = returnCode1;
                                  }
                              ...
                              if ( !runt1Method1.equals(runt1Method2) ) {
                                  log3("ERROR: !runt1Method1.equals(runt1Method2)");
                                  expresult = returnCode1;
                              }
                              if ( !runMethod1.equals(runMethod2) ) {
                                  log3("ERROR: !runMethod1.equals(runMethod2)");
                                  expresult = returnCode1;
                              }
