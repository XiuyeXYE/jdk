 ...
 * <code>com.sun.jdi.StackFrame.thisObject()</code>             <BR>
 * complies with its spec when the frame represents             <BR>
 * a non-native, non-static method.                             <BR>
 * <BR>
 ...
 *    StackFrame.thisObject() is invoked and                            <BR>
 *    its returned value is casted to ThreadReference.                  <BR>
 *    Since the thread is not resumed yet, neither                      <BR>
 *    InvalidStackFrameException no ClassCastException must be thrown.  <BR>
...
public class thisobject001 {
    ...
    public static int run (String argv[], PrintStream out) {
        return new thisobject001().runThis(argv, out);
    }
    ...
    private String testedClassName =
        "nsk.jdi.StackFrame.thisObject.Threadthisobject001a";

    //String mName = "nsk.jdi.StackFrame.thisObject";
    ...
    //------------------------------------------------------  testing section
        ...
        pipe.println("newcheck");
            ...
            label1: {
                ...
                log2("      getting stackFrame.thisObject() which must not be null");
                try {
                    thisobj = stackFrame.thisObject();
                    if (thisobj == null) {
                        log3("ERROR:  stackFrame.thisObject()==null for non-static method");
                    expresult = returnCode1;
                    break label1;
                    }
                    log2("      casting ObjectReference to ThreadReference");
                    ThreadReference t = (ThreadReference) thisobj;
                } catch ( InvalidStackFrameException e ) {
                    log3("ERROR:  InvalidStackFrameExceprtion before the thread is resumed");
                    expresult = returnCode1;
                    break label1;
                } catch ( ClassCastException e ) {
                    log3("ERROR:  ClassCastException for ThreadReference t = (ThreadReference) thisobj;");
                    expresult = returnCode1;
                    break label1;
                }
                ...
                log2("      checking up throwing InvalidStackFrameExceprtion after the thread2 is resumed");
                try {
                    thisobj = stackFrame.thisObject();
