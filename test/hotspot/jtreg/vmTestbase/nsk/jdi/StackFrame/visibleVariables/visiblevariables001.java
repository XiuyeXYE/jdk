 ...
 * <code>com.sun.jdi.StackFrame.visibleVariables()</code>       <BR>
 ...
 * 1) StackFrame.visiblevariables() is used to check up                 <BR>
 * that returned List contains expected number and names                <BR>
 * of visible variables.                                                <BR>
 * 2) After resuming the thread2, the tested method is invoked          <BR>
 *    second time and InvalidStackFrameException must be thrown.        <BR>
 * <BR>
 */

public class visiblevariables001 {
    ...
    public static int run (String argv[], PrintStream out) {
        return new visiblevariables001().runThis(argv, out);
    }
    ...
    private String testedClassName =
        "nsk.jdi.StackFrame.visibleVariables.Threadvisiblevariables001a";
    ...
    //------------------------------------------------------  testing section
        ...
        pipe.println("newcheck");
            ...
            label1: {
                ...
                String        name0 = null;
                String        name1 = null;
                String  testedName0 = "vi_i1";
                String  testedName1 = "vi_bl1";

                List visiblevars = null;

                log2("      checking up names of visible variables in the List");
                try {
                    visiblevars = stackFrame.visibleVariables();
                    if (visiblevars.size() != 2) {
                        log3("ERROR: visiblevars.size() != 2  :" + visiblevars.size());
                        expresult = returnCode1;
                    } else {
                        name0 = ( (LocalVariable) visiblevars.get(0) ).name();
                        name1 = ( (LocalVariable) visiblevars.get(1) ).name();

                        if (name0.equals(testedName0)) {
                            if (!name1.equals(testedName1)) {
                                log3("ERROR: !name1.equals(testedName1)");
                                expresult = returnCode1;
                            }
                        } else if (name0.equals(testedName1)) {
                                if (!name1.equals(testedName0)) {
                                    log3("ERROR: !name1.equals(testedName0)");
                                    expresult = returnCode1;
                                }
                        } else {
                            log3("ERROR: !name0.equals(testedName1)");
                            expresult = returnCode1;
                        }
                    }
                } catch ( AbsentInformationException e ) {
                    log3("ERROR:  AbsentInformationException for stackFrame.visiblevariables()");
                    expresult = returnCode1;
                ...
                try {
                    ...
                    log2("     :  InvalidStackFrameException is thrown:  OK");
                }
            }
