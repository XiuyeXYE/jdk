 ...
 * <code>com.sun.jdi.StackFrame.location()</code>               <BR>
 ...
 *    two sequential invokations of the method StackFrame.location()    <BR>
 *    are executed. Since the thread2 is not resumed yet,               <BR>
 *    InvalidStackFrameException must not be thrown on                  <BR>
 *    on second invokation, and both returned Location objects          <BR>
 *    must have the same values returned by invokations on them         <BR>
 *    methods codeIndex() and lineNumber();                             <BR>
 * 2) The debugger gets two mirrors of current method in the debuggee   <BR>
 *    one directly with Location.Method(), and another one with         <BR>
 *    VirtualMachine.classesByName().get().methodsByName().get(),       <BR>
 *    and checks up that both mirrors are equal to each other.          <BR>
 * 3) After resuming the thread2, the method is invoked one more        <BR>
 *    time and InvalidStackFrameException must be thrown.               <BR>
 *  <BR>
 */
...
public class location001 {
    ...
    private String testedClassName =
        "nsk.jdi.StackFrame.location.Threadlocation001a";

    //String mName = "nsk.jdi.StackFrame.location";
        ...
        pipe.println("newcheck");
           ...
           //BreakpointRequest breakpRequest3 = null;
            ...
            label1: {
                ...
                log2("      getting two Location objects, 1 and 2");
                try {
                    stackFrame = thread2.frame(0);
                    location1 = stackFrame.location();
                    location2 = stackFrame.location();
                } catch ( InvalidStackFrameException e1 ) {
                    expresult = returnCode1;
                    log3("ERROR:  InvalidStackFrameException before thread2 is resumed");
                    break label1;
                ...
                log2("      checking up that location1&2 have the same codeIndex and lineNumber");
                if (  location1.codeIndex()  != location2.codeIndex() ||
                      location1.lineNumber() != location2.lineNumber() ) {
                    log3("ERROR: codeIndexes or lineNumbers are not equal");
                    log3("     line1 # = " + location1.lineNumber() +
                         "  line2 # = " + location2.lineNumber());
                    expresult = returnCode1;
                    break label1;
                }
                ...
                if (m == null) {
                    log3("ERROR: m == null for: m = location1.method()");
                    expresult = returnCode1;
                } else if ( !m.equals( (Method) testedclass.methodsByName(breakpointMethod1).get(0)) ) {
                    log3("ERROR: two method's mirrors are not equal");
                    expresult = returnCode1;
                } else
                ...
                try {
                    location1 = stackFrame.location();
                    expresult = returnCode1;
                    log3("ERROR:  no InvalidStackFrameException after thread2 is resumed");
                } catch ( InvalidStackFrameException e ) {
                    log2("     :  InvalidStackFrameException after thread2 is resumed");
                }
