 ...
 *   4. Check that only correspondent function works and functions of
 ...
 */
static int checkGetObjectMonitorUsage(int positive) {
...
agentProc(jvmtiEnv* jvmti, JNIEnv* agentJNI, void* arg) {
    ...
    if (!checkRedefineClasses())
        nsk_jvmti_setFailStatus();
    if (!checkGetObjectMonitorUsage(NSK_TRUE))
        nsk_jvmti_setFailStatus();
    ...
    if (!checkGetObjectMonitorUsage(NSK_FALSE))
        nsk_jvmti_setFailStatus();
 ...
 */
#ifdef STATIC_BUILD
JNIEXPORT jint JNICALL Agent_OnLoad_cm01t016(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNICALL Agent_OnAttach_cm01t016(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNI_OnLoad_cm01t016(JavaVM *jvm, char *options, void *reserved) {
