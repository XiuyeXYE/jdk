    ...
    if (name != NULL && (strcmp(name, CLASS_NAME) == 0)) {
        ...
        if (!NSK_JVMTI_VERIFY(jvmti_env->Allocate(class_data_len, &klass_bytes)))
            ...
            if (!NSK_VERIFY(!found_1)) {
                NSK_COMPLAIN1("magic number 0x%x unexpectedly found\n",
                    magicNumber_1);
                nsk_jvmti_setFailStatus();
            }
...
#ifdef STATIC_BUILD
JNIEXPORT jint JNICALL Agent_OnLoad_ma07t001a(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNICALL Agent_OnAttach_ma07t001a(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNI_OnLoad_ma07t001a(JavaVM *jvm, char *options, void *reserved) {
