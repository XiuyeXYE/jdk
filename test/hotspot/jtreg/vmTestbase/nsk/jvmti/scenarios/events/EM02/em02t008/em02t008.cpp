        ...
        switch (step) {
            case 1:
                isExpected = ((curr == JVMTI_EVENT_VM_INIT)
                                || (curr == JVMTI_EVENT_EXCEPTION)
                                || (curr == JVMTI_EVENT_EXCEPTION_CATCH));
                break;

            case 2:
                isExpected = ((curr == JVMTI_EVENT_EXCEPTION)
                                || (curr == JVMTI_EVENT_EXCEPTION_CATCH));
                break;
...
cbException(jvmtiEnv *jvmti_env, JNIEnv* jni_env, jthread thread,
    ...
    if (!strncmp(info_ptr.name,"em02t008", 8)) {
        NSK_DISPLAY1("\tThread: %s\n", info_ptr.name);
...
void JNICALL
cbNewException(jvmtiEnv *jvmti_env, JNIEnv* jni_env, jthread thread,
                jmethodID method, jlocation location, jobject exception,
                jmethodID catch_method, jlocation catch_location) {
...
void JNICALL
cbNewExceptionCatch(jvmtiEnv *jvmti_env, JNIEnv* jni_env, jthread thread,
                jmethodID method, jlocation location, jobject exception) {
...
static bool setCallBacks(int step) {
    ...
    switch (step) {
        ...
        case 2:
            ...
            eventCallbacks.Exception                 = cbNewException;
            eventCallbacks.ExceptionCatch            = cbNewExceptionCatch;
            break;
...
#ifdef STATIC_BUILD
JNIEXPORT jint JNICALL Agent_OnLoad_em02t008(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNICALL Agent_OnAttach_em02t008(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNI_OnLoad_em02t008(JavaVM *jvm, char *options, void *reserved) {
