...
int checkEvents(int step) {
    ...
    for (i = 0; i < JVMTI_EVENT_COUNT; i++) {
        ...
        switch (step) {
            case 1:
                isExpected = ((curr == JVMTI_EVENT_VM_INIT)
                                || (curr == JVMTI_EVENT_METHOD_ENTRY)
                                || (curr == JVMTI_EVENT_METHOD_EXIT));
                break;

            case 2:
                isExpected = ((curr == JVMTI_EVENT_METHOD_ENTRY)
                                || (curr == JVMTI_EVENT_METHOD_EXIT));
                break;
        ...
        if (isExpected) {
            if (curr == JVMTI_EVENT_VM_INIT || curr == JVMTI_EVENT_VM_DEATH) {
...
void handler1(jvmtiEnv *jvmti_env, jvmtiEvent event, jmethodID method) {
...
cbMethodEntry(jvmtiEnv *jvmti_env, JNIEnv* jni_env, jthread thread,
    ...
    handler1(jvmti_env, JVMTI_EVENT_METHOD_ENTRY, method);
}
...
cbMethodExit(jvmtiEnv *jvmti_env, JNIEnv* jni_env, jthread thread,
    ...
    handler1(jvmti_env, JVMTI_EVENT_METHOD_EXIT, method);
}

void handler2(jvmtiEnv *jvmti_env, jvmtiEvent event, jmethodID method) {
...
void JNICALL
cbNewMethodEntry(jvmtiEnv *jvmti_env, JNIEnv* jni_env, jthread thread,
                jmethodID method) {

    handler2(jvmti_env, JVMTI_EVENT_METHOD_ENTRY, method);
}

void JNICALL
cbNewMethodExit(jvmtiEnv *jvmti_env, JNIEnv* jni_env, jthread thread,
                jmethodID method, jboolean was_popped_by_exception,
                jvalue return_value) {

    handler2(jvmti_env, JVMTI_EVENT_METHOD_EXIT, method);
}
...
static bool setCallBacks(int step) {
    ...
    switch (step) {
        ...
        case 2:
            ...
            eventCallbacks.MethodEntry               = cbNewMethodEntry;
            eventCallbacks.MethodExit                = cbNewMethodExit;
            break;
...
#ifdef STATIC_BUILD
JNIEXPORT jint JNICALL Agent_OnLoad_em02t009(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNICALL Agent_OnAttach_em02t009(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNI_OnLoad_em02t009(JavaVM *jvm, char *options, void *reserved) {
