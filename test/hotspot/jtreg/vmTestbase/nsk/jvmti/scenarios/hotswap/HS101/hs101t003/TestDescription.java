 ...
 * @test
 *
 * @summary converted from VM Testbase nsk/jvmti/scenarios/hotswap/HS101/hs101t003.
 ...
 *     Periodically hotswap class(es) with an EMCP version in asynchronous
 *     manner from a JVMTI agent's thread in accordance with specified time
 *     interval. The VM is provoked to be switched from compiled mode to
 *     interpreted and back by a tight loop in a test (compiled mode) and
 *     by enabling/disabling SingleStep event (interpreted mode).
 * COMMENTS
 *
 * @library /vmTestbase
 *          /test/lib
