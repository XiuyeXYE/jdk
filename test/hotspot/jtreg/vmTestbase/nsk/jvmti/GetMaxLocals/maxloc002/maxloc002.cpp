...
#ifdef STATIC_BUILD
JNIEXPORT jint JNICALL Agent_OnLoad_maxloc002(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNICALL Agent_OnAttach_maxloc002(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNI_OnLoad_maxloc002(JavaVM *jvm, char *options, void *reserved) {
...
Java_nsk_jvmti_GetMaxLocals_maxloc002_check(JNIEnv *env, jclass cls) {
    jvmtiError err;
    jmethodID mid;
    jint max;
    ...
    err = jvmti->GetMaxLocals(mid, &max);
