...
#define MONITOR_WAIT_TIMEOUT ((jlong)10)
...
void JNICALL VMInit(jvmtiEnv *jvmti_env, JNIEnv *env, jthread thr) {
    ...
    if (printdump == JNI_TRUE) {
        printf(">>> waiting %d raw monitors\n", RAW_MONITORS_NUMBER);
    }

    for (i = 0; i < RAW_MONITORS_NUMBER; i++) {
        err = jvmti->RawMonitorWait(monitors[i], MONITOR_WAIT_TIMEOUT);
        if (err != JVMTI_ERROR_NONE) {
            printf("(RawMonitorWait#%d) unexpected error: %s (%d)\n",
                   i, TranslateError(err), err);
            result = STATUS_FAILED;
        }
    }
...
#ifdef STATIC_BUILD
JNIEXPORT jint JNICALL Agent_OnLoad_rawmnwait001(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNICALL Agent_OnAttach_rawmnwait001(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNI_OnLoad_rawmnwait001(JavaVM *jvm, char *options, void *reserved) {
...
JNIEXPORT jint JNICALL Java_nsk_jvmti_RawMonitorWait_rawmnwait001_check(JNIEnv *env, jclass cls) {
    jvmtiError err;
    char name[32];
    int i;
