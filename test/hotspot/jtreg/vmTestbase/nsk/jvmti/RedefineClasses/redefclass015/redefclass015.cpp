...
#ifdef STATIC_BUILD
JNIEXPORT jint JNICALL Agent_OnLoad_redefclass015(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNICALL Agent_OnAttach_redefclass015(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNI_OnLoad_redefclass015(JavaVM *jvm, char *options, void *reserved) {
...
Java_nsk_jvmti_RedefineClasses_redefclass015_check(JNIEnv *env, jclass cls,
    ...
    if (err == JVMTI_ERROR_MUST_POSSESS_CAPABILITY &&
        ...
        printf("Error expected: JVMTI_ERROR_FAILS_VERIFICATION,\n");
        printf("\tactual: %s (%d)\n", TranslateError(err), err);
