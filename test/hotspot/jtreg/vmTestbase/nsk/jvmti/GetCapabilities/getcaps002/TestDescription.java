 ...
 * @test
 *
 * @summary converted from VM Testbase nsk/jvmti/GetCapabilities/getcaps002.
 * VM Testbase keywords: [quick, jpda, jvmti, noras]
 * VM Testbase readme:
 * DESCRIPTION
 *     This JVMTI test exercises JVMTI thread function GetCapabilities().
 *     This test checks that GetCapabilities() returns structure with no
 *     capabilities set if no AddCapabilities() was invoked before.
 ...
 *     The test passes if GetCapabilities() returns structure with no
 *     capabilities set in all checks.
 *     If the last check (in VM_DEATH callback) fails, then C-language exit()
 *     function is used to force VM exit with fail status 97.
 * COMMENTS
 *     For more testcases see tests for AddCapabilities().
