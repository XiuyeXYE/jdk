 ...
 * @test
 *
 * @summary converted from VM Testbase nsk/jvmti/SetEventCallbacks/setevntcallb002.
 ...
 *     This test checks that if SetEventCallbacks() is invoked with NULL
 *     for event callbacks, then no events will be generated and no callbacks
 *     will be invoked.
 *     In Agent_OnLoad() the test sets event callbacks for VM_INIT,
 *     THREAD_START and THREAD_END events, but enables only VM_INIT event.
 *     In VM_INIT callback the test sets NULL for event callbacks and
 *     enables other events (THREAD_START and THREAD_END).
 ...
 *      -agentlib:setevntcallb002=-waittime=5
 *      nsk.jvmti.SetEventCallbacks.setevntcallb002
 */
