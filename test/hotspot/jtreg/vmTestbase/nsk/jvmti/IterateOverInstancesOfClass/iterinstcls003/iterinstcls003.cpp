...
#define DEBUGEE_CLASS_NAME      "nsk/jvmti/IterateOverInstancesOfClass/iterinstcls003"
#define ROOT_OBJECT_CLASS_NAME  "nsk/jvmti/IterateOverInstancesOfClass/iterinstcls003RootTestedClass"
#define ROOT_OBJECT_CLASS_SIG   "L" ROOT_OBJECT_CLASS_NAME ";"
#define CHAIN_OBJECT_CLASS_NAME "nsk/jvmti/IterateOverInstancesOfClass/iterinstcls003TestedClass"
...
static int checkTestedObjects(jvmtiEnv* jvmti, JNIEnv* jni,
    ...
    for (i = 0; i < chainLength; i++) {
        ...
        if (objectDescList[i].found > 0
            ...
            NSK_COMPLAIN2("Reachable tagged object was iterated:\n"
                          "#   tag:      %ld\n"
                          "#   iterated: %d times\n",
    ...
    for (i = 0; i < chainLength; i++) {
        ...
        if (objectDescList[i + chainLength].found <= 0
            ...
            NSK_COMPLAIN2("Not collected unreachable tagged object was iterated:\n"
                          "#   tag:      %ld\n"
                          "#   iterated: %d times\n",
                            (long)objectDescList[i + chainLength].tag,
                            objectDescList[i + chainLength].found);
    ...
    if (tag_ptr == NULL) {
        ...
        if (*tag_ptr == 0) {
            ...
            if (found <= 0) {
                ...
                NSK_COMPLAIN3("Known tagged object passed to heapObjectCallback:\n"
                              "#   tag:            %ld\n"
                              "#   class_tag:      %ld\n"
                              "#   size:           %ld\n",
                                (long)*tag_ptr,
...

    {
        ...
        NSK_DISPLAY0(">>> Obtain and tag tested objects from debugee class\n");
        {
            if (!NSK_VERIFY(getTestedObjects(jvmti, jni, chainLength,
                                        &objectsCount, &objectDescList, &testedClass)))
                return;
        }
        ...
        NSK_DISPLAY0(">>> Disable OBJECT_FREE event and check if untagged objects were iterated:\n");
        {
            jvmtiEvent event = JVMTI_EVENT_OBJECT_FREE;
...
#ifdef STATIC_BUILD
JNIEXPORT jint JNICALL Agent_OnLoad_iterinstcls003(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNICALL Agent_OnAttach_iterinstcls003(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNI_OnLoad_iterinstcls003(JavaVM *jvm, char *options, void *reserved) {
